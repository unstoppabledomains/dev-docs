...
{% partial file="/web3/_partials/_new_tld_warning.md" /%}
...
When you encounter errors using any resolution method (Resolution Libraries, Resolution Service API, CLI, Blockchain Calls), handle them according to the error type rather than generically handling them or passing them silently. Please see the [Resolution API](/web3/apis/resolution/openapi#operation/StatusController.listSupportedTlds) docs for language-specific error handling guides and library error codes you might encounter during integration.
