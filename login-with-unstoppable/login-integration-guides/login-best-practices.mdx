...
Here are the best practices for integrating your application with Login with Unstoppable.

## Accessing User Info

Authorizations are stored inside `localStorage`, so any identically configured `UAuth` instance has access to the same users.
Integrations using [@uauth/js](/login-with-unstoppable/libraries/uauth-js.md) or any UAuth middleware package that uses it, you can access the user information by instantiating a new `UAuth` object with the same client options and calling the `user()` method.

You can use the access methods below to obtain the user information.
...
## Getting the Authorization Account
